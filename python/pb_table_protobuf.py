#!/usr/bin/env python3
"""
Full protobuf-table Python implementation with cross-language compatibility

This implementation uses proper Protocol Buffer Row messages to maintain
complete wire format compatibility with the JavaScript version.
...
def _create_simple_row_data(header_fields: List[Dict[str, Any]], row_data: List[Any]) -> Dict[str, Any]:
    """Create a simple dictionary representation of row data."""
    result = {}
    for i, field_def in enumerate(header_fields):
        field_name = field_def['name']
        if i < len(row_data):
            result[field_name] = row_data[i]
        else:
            result[field_name] = None
    return result

def _extract_row_data_from_dict(header_fields: List[Dict[str, Any]], row_dict: Dict[str, Any]) -> List[Any]:
    """Extract row data from dictionary back to array format."""
    result = []
    for field_def in header_fields:
        field_name = field_def['name']
        result.append(row_dict.get(field_name))
    return result

# Global factory instance
_row_factory = DynamicRowMessageFactory()
...
def encode_table(obj: Dict[str, Any], callback: Optional[Callable] = None) -> bytes:
    """Encode table data (array format) to protobuf bytes using proper Row messages."""
    try:
        ...
        # Create dynamic Row and Data message types
        RowMessage, DataMessage = _row_factory.create_row_message_type(obj_with_stats['header'])
        
        # Encode each row using proper protobuf Row messages
        data_buffer = io.BytesIO()
        
        for row_index, row_data in enumerate(obj['data']):
            # Create Data message with single Row
            data_msg = DataMessage()
            row_msg = data_msg.data.add()
            
            # Set field values with transforms applied
            for col, field_def in enumerate(obj['header']):
                ...
                # Set field value in Row message
                setattr(row_msg, field_name, value)
            
            # Serialize the Data message
            serialized_data = data_msg.SerializeToString()
...
def decode_table(buffer: bytes, callback: Optional[Callable] = None) -> Dict[str, Any]:
    """Decode table data (array format) from protobuf bytes using proper Row messages."""
    try:
        # Decode header using compiled proto
        header_obj, data_offset = _decode_header_delimited(buffer)
        
        # Create dynamic Row and Data message types
        RowMessage, DataMessage = _row_factory.create_row_message_type(header_obj['header'])
        ...
        while offset < len(buffer):
            ...
            # Read and parse Data message
            message_data = buffer[new_offset:new_offset + length]
            data_msg = DataMessage()
            data_msg.ParseFromString(message_data)
            
            # Extract row data from the Data message
            for row_msg in data_msg.data:
                # Convert Row message to array format with reverse transforms
                restored_row = []
...
if __name__ == "__main__":
    ...
    print("Testing full protobuf-table Python implementation...")
    ...
    print("Success! Full protobuf implementation working.")
